...
public class EndEffectorIOTalonFX implements EndEffectorIO {
  ...
  EndEffectorIOTalonFX() {
    ...
    BaseStatusSignal.setUpdateFrequencyForAll(
        50.0, endPosition, endVelocity, endAppliedVolts, endCurrent);
  ...
  public void updateInputs(EndEffectorInputsAutoLogged inputs) {
    ...
    inputs.endConnected = endConnectedDebounce.calculate(endStatus.isOK());
    inputs.endMotorPositionRad = Units.rotationsToRadians(endPosition.getValueAsDouble());
    inputs.endMotorVelocityRadPerSec =
        Units.rotationsToRadians(endVelocity.getValueAsDouble());
    inputs.endAppliedVolts = endAppliedVolts.getValueAsDouble();
    inputs.endCurrentAmps = endCurrent.getValueAsDouble();
  }
